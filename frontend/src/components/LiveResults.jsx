...
const LiveResults = () => {
    ...
    const [timer, setTimer] = useState(() => {
        // initial placeholder; will be set properly in effect
        return 0;
    });
    ...
    useEffect(() => {
    const onPollEnded = () => {
        setCanAskNew(true); // ✅ show Ask New Question
    };
    ...
    useEffect(() => {
        // Determine expiry timestamp (ms). Priority: poll.expiryTime -> sessionStorage -> poll.timeLimit
        const storedExpiry = sessionStorage.getItem("poll_expiry_time");
        const expiryFromPoll = poll?.expiryTime || null;
        let expiry = null;

        if (expiryFromPoll) {
            expiry = Number(expiryFromPoll);
            sessionStorage.setItem("poll_expiry_time", expiry.toString());
        } else if (storedExpiry) {
            expiry = Number(storedExpiry);
        } else if (poll?.timeLimit) {
            expiry = Date.now() + Number(poll.timeLimit) * 1000;
            sessionStorage.setItem("poll_expiry_time", expiry.toString());
        } else {
            // fallback 60s
        ...
        const computeRemaining = () => Math.max(0, Math.ceil((expiry - Date.now()) / 1000));
